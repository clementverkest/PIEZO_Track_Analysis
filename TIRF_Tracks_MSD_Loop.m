...
% : allspots csv table, spots and tracks (optional) csv tables, Track in
% xml format to be read by TraJclassifier
% TrajClassifier tables required: all four track classes and Parents tables
% All tables stored in one folder per cell
% Requires : ExtractAndAssignTracks.m, PlotAllTracks.m, calculate_msd.m,
% average_msd.m, calculate_diffusion_coefficient.m, GaussFit2D.m

% Select a folder and loop through subfolders containing
% trajectories/clusters tables. One subfolder per cell. 

% Make sure beforehand that each subfolder contains required tables and tif image
% Make sure that only subfolder that contain required tables are present in
% the selected parent folder

%%

close all
clearvars -except all_tracks all_diff allResults pixelSize
clc
...
%
pixelSize = 0.110 % indicate the pixel size in your images 
% Must match with the one used when generating trackmate tables !


% Loop through all folders (for one condition)
folderList = uigetdir(pwd, 'Select parent folder containing all experiment folders');
subFolders = dir(folderList);
subFolders = subFolders([subFolders.isdir]);
subFolders = subFolders(~ismember({subFolders.name}, {'.', '..'}));


% Create a main results folder
mainResultsFolder = fullfile(folderList, 'All_Results');
if ~exist(mainResultsFolder, 'dir')
    mkdir(mainResultsFolder);
end

% Create a structure that will keep all the track info tables for each cell
    all_tracks = struct();
     all_tracks.confined_tracks = {};
    all_tracks.directed_tracks = {};
    all_tracks.diffusion_tracks = {};
    all_tracks.subdiffusion_tracks = {};

% Option - store all the diffusion coefficient for each cell
    all_diff = struct();
    all_diff.confined_tracks = {};
    all_diff.directed_tracks = {};
    all_diff.diffusion_tracks = {};
    all_diff.subdiffusion_tracks = {};

% Loop start here
for folderIdx = 1:length(subFolders)  % option to access all the subfolders
    % for folderIdx = 1:1 %  option to select only given subfolders

    currentFolder = fullfile(subFolders(folderIdx).folder, subFolders(folderIdx).name);
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Loop through folder and Load trajectories/spots data information to make tables
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Specify the folder where the files are
% myfolderpath = uigetdir();
myfolderpath = currentFolder
% Get a list of all files in the folder with the desired file name pattern,
% CSV files
filePattern = fullfile(myfolderpath, '**/*.csv'); % Change to whatever pattern you need.
theFiles = dir(filePattern);
imagePattern = fullfile(myfolderpath, '**/*.tif');
theImage = dir(imagePattern);
% XML files
...
% Alternative loading trajectories files: individual loading
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Prepare for assigning and extracting subtracks %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Extract cluster intensity and size for each tracks %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:numel(subdiffusion_tracks)
    subdiffusion_msd{i} = calculate_msd(subdiffusion_tracks{i}, 40);
end

% Calculate MSD for normal diffusion tracks
for i = 1:numel(diffusion_tracks)
    diffusion_msd{i} = calculate_msd(diffusion_tracks{i}, 40);
end

% Calculate MSD for directed tracks
for i = 1:numel(directed_tracks)
    directed_msd{i} = calculate_msd(directed_tracks{i}, 40);
end

% Calculate MSD for confined tracks
for i = 1:numel(confined_tracks)
    confined_msd{i} = calculate_msd(confined_tracks{i}, 40);
end
...
% Calculate diffusion coefficients for each track type
...
all_diff.confined_tracks.(['cell_' num2str(folderIdx),'_' (testedconstruct)]) = confined_coeffs;
all_diff.diffusion_tracks.(['cell_' num2str(folderIdx),'_' (testedconstruct)]) = diffusion_coeffs;
all_diff.directed_tracks.(['cell_' num2str(folderIdx),'_' (testedconstruct)]) = directed_coeffs;
all_diff.subdiffusion_tracks.(['cell_' num2str(folderIdx),'_' (testedconstruct)]) = subdiffusion_coeffs;
...
% MSD curves
    ...
    foldername = fullfile(mainResultsFolder, ['Results_', subFolders(folderIdx).name]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~isempty(results1{i})
        % Extract the first element if the cell is not empty
        cluster_diameter(i) = results1{i}(1);
    else
        % Optionally, handle the empty cell case (e.g., assign NaN or skip)
        cluster_diameter(i) = NaN; % Assign NaN for empty cells
    end

        end
        diameter_conf = mean(cluster_diameter, 'omitnan');
    ...
    if ~isempty(results2{i})
        ...
        diameter_dir = mean(cluster_diameter, 'omitnan');
        end
    ...
    if ~isempty(results3{i})
        ...
        diameter_dif = mean(cluster_diameter, 'omitnan');
    ...
    if ~isempty(results4{i})
        ...
        diameter_subdif = mean(cluster_diameter, 'omitnan');
        %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
% Additional steps to extract and store results with all the data points from different cell

% Create array with all clusters diameters
...
for i = 1:numel(allResults)
    ...
    for fieldName = ["results1", "results2", "results3", "results4"]
        ...
        % Check if cellArray is empty
        if isempty(cellArray)
            continue; % Skip to the next field if cellArray is empty
        end
        
        % Loop through each cell in the cell array
        for j = 1:numel(cellArray)
            % Check if the cell is non-empty and contains a 1x2 array
            if ~isempty(cellArray{j}) && numel(cellArray{j}) >= 1
...
%% Extract all the diffusion values
% First, inspect the structure to understand its actual format
fields_to_extract = {'confined_tracks', 'directed_tracks', 'subdiffusion_tracks', 'diffusion_tracks'};

% Initialize an empty array
extracted_values = [];

% Loop through each field
for field_idx = 1:length(fields_to_extract)
    current_field = fields_to_extract{field_idx};
    
    % Get all cell names in the current field
    cell_names = fieldnames(all_diff.(current_field));
    
    % Loop through each cell
    for cell_idx = 1:numel(cell_names)
        % Get the current cell's data
        current_cell_data = all_diff.(current_field).(cell_names{cell_idx});
        
        % If it's a cell array, extract its values
        for inner_idx = 1:numel(current_cell_data)
            % Append value to extracted_values
            extracted_values = [extracted_values; current_cell_data{inner_idx}];
        end
    end
end

%% for mobile fraction
fields_to_extract = { 'directed_tracks', 'diffusion_tracks'};

% Initialize an empty array
extracted_values_mobile = [];
...
for field_idx = 1:length(fields_to_extract)
    ...
    for cell_idx = 1:numel(cell_names)
        ...
        for inner_idx = 1:numel(current_cell_data)
            % Append value to extracted_values
            extracted_values_mobile = [extracted_values_mobile; current_cell_data{inner_idx}];
        end
    end
end

%% For immobile fraction
fields_to_extract = { 'confined_tracks', 'subdiffusion_tracks'};

% Initialize an empty array
extracted_values_immobile = [];
...
for field_idx = 1:length(fields_to_extract)
    ...
    for cell_idx = 1:numel(cell_names)
        ...
        for inner_idx = 1:numel(current_cell_data)
            % Append value to extracted_values
            extracted_values_immobile = [extracted_values_immobile; current_cell_data{inner_idx}];
        end
    end
end


%% Extract info for intensity
fields_to_extract = { 'confined_tracks', 'subdiffusion_tracks'};

% Initialize an empty array
intensity_extracted_values_immobile = [];
...
for field_idx = 1:length(fields_to_extract)
    ...
    % Get all cell names in the current field
    cell_names = fieldnames(all_tracks.(current_field));
    
    % Loop through each cell
    for cell_idx = 1:numel(cell_names)
        % Get the current cell's data
        current_cell_data = all_tracks.(current_field).(cell_names{cell_idx});
        
        % If it's a cell array of tables
        for inner_idx = 1:numel(current_cell_data)
            % Get current table
            current_table = current_cell_data{inner_idx};
            
            % Get unique values in column 3
            unique_col3_values = unique(current_table{:, 3});
            
            % Loop through unique values
            for unique_idx = 1:size(unique_col3_values)
                % Find rows matching the unique value in column 3
                matching_rows = current_table{:, 3} == unique_col3_values(unique_idx);
                
                % Extract corresponding value from column 13
                col13_value = current_table{matching_rows, 13};
                
                % Store first value
                intensity_extracted_values_immobile = [intensity_extracted_values_immobile; col13_value(1)];
            end
        end
    end
end

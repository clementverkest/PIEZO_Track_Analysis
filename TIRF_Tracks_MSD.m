...
PlotTracks = false
PlotMSDAverage = false
PlotMSDIndividual = false
PlotImgOverlaywithTracks = false
CreateTracksMovie = false
ExportcsvTables = true
DoCustomGaussFitCluster = false

% Variables to adjust beforehand
%pixel size (in µm)
pixelSize = 0.110 % indicate the pixel size in your images. Must match with the one used when generating trackmate tables !
%Number of points/frames to plot for MSD graphs
...
%Generate empty results structure to keep data from different files
%(usefull only if loop)
all_tracks = struct();
...
if CreateTracksMovie

% Parameters
frameInterval; % XX s per frame - to be set before
pixelSize; %  XX µm per pixel - to be set before
        ...
        if any(validIdx)
            ...
            if any(inView)
                ...
                if length(x_visible) > 1
                    ...
                    plot(x_visible, y_visible, 'Color', 'red', 'LineWidth', 1);
                end
    ...
    for trackIdx = 1:numel(diffusion_tracks)
        ...
        if any(validIdx)
            ...
            if any(inView)
                ...
                if length(x_visible) > 1
                    % Plot the entire visible track segment with full opacity
                    plot(x_visible, y_visible, 'Color', 'yellow', 'LineWidth', 1);
                end
...
for trackIdx = 1:numel(confined_tracks)
        ...
        if any(validIdx)
            ...
            if any(inView)
                ...
                if length(x_visible) > 1
                    % Plot the entire visible track segment with full opacity
                    plot(x_visible, y_visible, 'Color', 'green', 'LineWidth', 1);
                end
...
for trackIdx = 1:numel(subdiffusion_tracks)
        ...
        if any(validIdx)
            ...
            if any(inView)
                ...
                if length(x_visible) > 1
                    % Plot the entire visible track segment with full opacity
                    plot(x_visible, y_visible, 'Color', 'blue', 'LineWidth', 1);
                end
...
% Retrieve data from Trackmate table
%
    DiameterMeans_conf = zeros(numel(confined_tracks), 1);
    for i = 1:numel(confined_tracks);
     currentTable = confined_tracks{i};   
    DiameterMeans_conf(i)=mean(currentTable.RADIUS, 'omitnan');
    end
    diameter_conf = mean(DiameterMeans_conf);

 %
DiameterMeans_dir = zeros(numel(directed_tracks), 1);
    for i = 1:numel(directed_tracks);
     currentTable = directed_tracks{i};   
    DiameterMeans_dir(i)=mean(currentTable.RADIUS, 'omitnan');
    end
    diameter_dir = mean(DiameterMeans_dir);

    %
    DiameterMeans_dif = zeros(numel(diffusion_tracks), 1);
    for i = 1:numel(diffusion_tracks);
     currentTable = diffusion_tracks{i};   
    DiameterMeans_dif(i)=mean(currentTable.RADIUS, 'omitnan');
    end
diameter_dif = mean(DiameterMeans_dif);

%
 DiameterMeans_subdif = zeros(numel(subdiffusion_tracks), 1);
    for i = 1:numel(subdiffusion_tracks);
     currentTable = subdiffusion_tracks{i};   
    DiameterMeans_subdif(i)=mean(currentTable.RADIUS, 'omitnan');
    end
diameter_subdif = mean(DiameterMeans_subdif);
%
